            ...
            <div class="mb-3 row">
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <select name="tool_category_id" class="form-control" required>
                            <option value="">Pilih Kategori</option>
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}"
                                    {{ $tool->tool_category_id == $category->id ? 'selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="mb-3 row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <button type="button" class="mt-2 btn btn-danger" id="remove-image"
                            style="display: {{ $tool->image ? 'block' : 'none' }};" onclick="removeImage()">
                            <i class="fas fa-circle-xmark"></i>
                        </button>
                    </div>
                    <div class="card">
                        <div class="card-body">
                            <h5 class="card-title">Gambar Terpilih</h5>
                            @if (filter_var($tool->image_path, FILTER_VALIDATE_URL))
                                <img src="{{ $tool->image_path }}" alt="{{ $tool->name }}" class="img-fluid h-100 w-100"
                                    style="max-height: 260px; display: block; width: 100%; object-fit: contain;">
                            @else
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="url" name="image_url" id="image_url" class="form-control"
                            placeholder="https://example.com/image.jpg" value="{{ $tool->image_url }}">
                    </div>
                </div>
            </div>
